...
print_message "Checking Node.js and npm for localtunnel..."
...
print_message "Installing localtunnel globally..."
...
while [[ $RETRY_COUNT -lt $MAX_RETRIES && $SERVER_STARTED == false ]]; do
    ...
    if [[ -n "$HTTP_SERVER_PID" ]]; then
        print_message "Starting localtunnel to http://localhost:$PORT..."
        
        lt --port "$PORT" > /tmp/localtunnel_$$.log 2>&1 &
        LOCALTUNNEL_PID=$!
        
        sleep 10
        
        TUNNEL_URL=$(grep -o 'https://[^ ]*\.loca\.lt' /tmp/localtunnel_$$.log | head -n 1)
        
        if [[ -n "$TUNNEL_URL" ]]; then
            print_success "Localtunnel established at: $TUNNEL_URL"
            SERVER_STARTED=true
        else
            print_warning "Localtunnel not established yet. Waiting longer..."
            ...
            if [[ -n "$TUNNEL_URL" ]]; then
                ...
                print_error "Failed to establish localtunnel. Stopping services and trying another port."
...
# Wait for Ctrl+C
trap 'echo -e "${YELLOW}Stopping servers...${NC}"; if [[ -n "$HTTP_SERVER_PID" ]]; then kill $HTTP_SERVER_PID 2>/dev/null; fi; if [[ -n "$LOCALTUNNEL_PID" ]]; then kill $LOCALTUNNEL_PID 2>/dev/null; fi; echo -e "${GREEN}Servers stopped.${NC}"' INT
wait
